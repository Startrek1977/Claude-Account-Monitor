...
Codex Account Monitor is a .NET 8.0 WPF desktop application for monitoring and tracking Codex account settings and configuration. It is a Windows-only application using WPF UI framework.

## Project Structure

```
src/
├── Codex-Account-Monitor.sln          # Solution file
├── Codex-Account-Monitor/              # Main WPF application
│   ├── Codex-Account-Monitor.csproj
│   ├── App.xaml(.cs)                   # Application entry point
│   └── MainWindow.xaml(.cs)            # Main window
└── Codex-Account-Monitor.Tests/        # xUnit test project
    ├── Codex-Account-Monitor.Tests.csproj
...
```bash
# Build the solution
dotnet build Codex-Account-Monitor.sln

# Build in Release mode
dotnet build Codex-Account-Monitor.sln -c Release

# Run the application
dotnet run --project Codex-Account-Monitor/Codex-Account-Monitor.csproj

# Watch mode (auto-rebuild on changes)
dotnet watch run --project Codex-Account-Monitor.sln
```
...
```bash
# Run all tests
dotnet test Codex-Account-Monitor.sln

# Run tests with detailed output
dotnet test Codex-Account-Monitor.sln --logger "console;verbosity=detailed"

# Run tests with code coverage
dotnet test Codex-Account-Monitor.sln /p:CollectCoverage=true

# Run a specific test class
dotnet test Codex-Account-Monitor.sln --filter "FullyQualifiedName~MainWindowTests"
```
